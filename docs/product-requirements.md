...
- [ ] 사용자를 생성·조회·수정할 수 있다.
- [ ] 사용자의 대화방을 생성·조회·삭제할 수 있다.
- [ ] 개발 질문을 AI에 전달하고 답변을 화면에 표시할 수 있다.
- [ ] 대화 내용이 PostgreSQL에 저장된다.
- [ ] 감지 기술과 부족한 개념이 학습 상태에 반영된다.
- [ ] 기술별 이해도와 다음 추천 학습을 확인할 수 있다.
- [ ] 확인 질문에 답하고 AI 평가를 받을 수 있다.
- [ ] 평가 결과가 학습 상태와 복습 대상에 반영된다.
- [ ] 프론트엔드와 백엔드가 실제 API로 연결된다.
- [ ] Docker Compose로 PostgreSQL을 실행할 수 있다.
- [ ] 주요 서비스 규칙에 단위 테스트가 있다.
- [ ] 주요 저장·조회 흐름에 PostgreSQL 통합 테스트가 있다.
- [ ] README만으로 로컬 실행과 테스트가 가능하다.

## 11. 미확정 결정사항

구현 전에 해당 작업 범위에서만 결정합니다.

1. 관심 기술의 저장 방식과 사용자 수정 API 계약
2. 기본 사용자 자동 생성과 닉네임 생성 방식 중 MVP 선택
3. DB 스키마 관리에 `data.sql`만 사용할지 migration 도구를 도입할지
4. OpenAI 공식 연동 방식과 구조화 출력 스키마
5. AI 원문 분석 결과를 JSON 컬럼으로 저장할지 텍스트로 저장할지
6. 단순 점수 증감값과 상태 전이 기준
